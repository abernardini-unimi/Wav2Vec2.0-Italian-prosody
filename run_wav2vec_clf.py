...
try:
    ...
    print(f"Could not set audio backend to soundfile: {e}")
...
def main():
    ...
    is_regression = True
    num_labels = 3
    logger.info(f"*** Arousal-Valence-Dominance Regression (3 outputs) ***")
    ...
    def speech_file_to_array_fn(path):
        import soundfile as sf  # Usiamo soundfile direttamente per stabilità
        
        # 1. Leggi il file audio (ritorna un numpy array)
        # SoundFile ritorna: (Time, Channels) oppure (Time,) se mono
        speech_array, sampling_rate = sf.read(path)
        
        # 2. Converti in PyTorch Tensor e assicurati che sia float
        speech_array = torch.from_numpy(speech_array).float()
        
        # 3. Normalizza le dimensioni per PyTorch: (Channels, Time)
        if speech_array.ndim == 1:
            # Se è mono (1D), aggiungi la dimensione canale -> (1, Time)
            speech_array = speech_array.unsqueeze(0)
        else:
            # Se è stereo/multi (Time, Channels), trasponi -> (Channels, Time)
            speech_array = speech_array.t()
            
        # 4. Converti in Mono (mixdown) se ci sono più canali
        if speech_array.shape[0] > 1:
            speech_array = torch.mean(speech_array, dim=0, keepdim=True)
            
        # 5. Ricampiona se necessario (usando torchaudio.functional)
        if sampling_rate != target_sampling_rate:
            speech_array = F.resample(speech_array, sampling_rate, target_sampling_rate)
            
        # 6. Ritorna un array numpy "squeezed" (senza dimensioni extra) come richiesto dal trainer
    ...
    def preprocess_function(examples):
        ...
        # MODIFICA: Invece di label_to_id, estraiamo i 3 valori float
        # Assumiamo che nel dataset ci siano le colonne 'arousal', 'valence', 'dominance'
        # Se i nomi sono diversi nel tuo CSV, aggiornali qui.
        target_list = []
        for i in range(len(examples[input_column_name])):
            # Esempio: prendiamo i valori e li mettiamo in una lista [A, V, D]
            val_a = float(str(examples['arousal'][i]).replace(',', '.'))
            val_v = float(str(examples['valence'][i]).replace(',', '.'))
            val_d = float(str(examples['dominance'][i]).replace(',', '.'))
            target_list.append([val_a, val_v, val_d])

        result = feature_extractor(speech_list, sampling_rate=target_sampling_rate)
        result["labels"] = target_list # Ora è una lista di liste di float
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        # label_ids shape: (n_samples, 3)
        
        # Calcolo CCC numpy-based per la validazione
        ...
        # Calcoliamo CCC separato per Arousal, Valence, Dominance
        ccc_a = concordance_correlation_coefficient(p.label_ids[:, 0], preds[:, 0])
        ccc_v = concordance_correlation_coefficient(p.label_ids[:, 1], preds[:, 1])
        ccc_d = concordance_correlation_coefficient(p.label_ids[:, 2], preds[:, 2])
        
        return {
            "ccc_arousal": ccc_a,
            "ccc_valence": ccc_v,
            "ccc_dominance": ccc_d,
            "ccc_mean": (ccc_a + ccc_v + ccc_d) / 3.0,
